...
Two honest qualifications attach to the cross-packet *refinement* of this picture. First, the blind spot's literal diagnostic — the cross-packet mass of the centered score — does **not** transfer to this task: a generalised modular-addition head outputs a near-delta, so its score-cubic *vanishes* as the model becomes correct (its total mass collapses by many orders of magnitude through the grok) and it does not separate ring from control. The ring structure lives in the input→output *map*, which the two registers read, not in the marginal output's score-cubic. Second, a *persistent* form of the same diagnostic — the cross-packet cubic of the logits' offset profile, which *grows* rather than vanishes as the model generalises — does separate cleanly on the composite modulus. Its **absolute cross-packet mass** accumulates through the grok and exceeds the control's by four orders of magnitude ($\approx 10^4\times$), while the structure-free control holds the baseline — the directional behaviour the conductor blind spot's trajectory conjecture predicts: cross-packet activation accumulating on a ring task and staying null on a matched label-permuted control. (The cross-packet *share* dips at the transition only because the total cubic mass grows even faster; the conjecture's quantity is the absolute cross-packet mass, and it grows.) We are precise about scope: the measurement is on the head's logit offset profile — a persistent transform that reflects the trained upstream features rather than the categorical head in isolation, consistent with the conjecture's own refinement that the asymmetry is depth-driven — so this is trajectory evidence in the predicted direction, not a from-scratch proof of the conjecture's mechanism.
